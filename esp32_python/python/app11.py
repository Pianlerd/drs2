...
ESP32_IP = "http://192.168.118.124" # ตรวจสอบให้แน่ใจว่า IP Address นี้ถูกต้อง
...
def sensor_reset():
    ...
    """
    print("[Sensor Trigger] IR detected → Resetting category_id 1–5")  # log สำหรับ debug
    ...
    try:
        ...
        # อัปเดต DB ให้เป็น 0 สำหรับ category_id 1–5
        cursor.execute("UPDATE tbl_bin SET value = 0 WHERE category_id IN (1, 2, 3, 4, 5)")
        conn.commit()

        # รายชื่อ Servo ที่จะสั่งให้ OFF
        servo_ids = [12, 13, 14, 26, 27]

        for sid in servo_ids:
            success = False
            for attempt in range(3):  # ลองใหม่ได้สูงสุด 3 ครั้ง
                try:
                    url = f"{ESP32_IP}/servo{sid}/off"
                    res = requests.get(url, timeout=5)
                    print(f"[Forced Update] Servo {sid} → OFF (attempt {attempt + 1})")
                    servo_cache[sid] = 0  # บันทึกค่า cache ให้ตรงกัน
                    success = True
                    break
                except requests.exceptions.RequestException as e:
                    print(f"[Retry] Servo {sid} → OFF failed (attempt {attempt + 1}): {e}")
                    time.sleep(0.5)

            if not success:
                print(f"[Error] Cannot control ESP32 for Servo {sid} after 3 attempts")

            time.sleep(0.3)  # พัก 300ms ต่อ servo

        return "[OK] Sensor reset success", 200
