...
def bin():
    ...
    if not conn:
        flash("Error connecting to the database.", 'danger')
        # *** If DB connection fails, display error on OLED (2 lines) ***
    ...
    if request.method == 'POST':
        ...
        if action == 'add_disquantity':
            # Action to prepare for disquantity increment
            ...
            try:
                ...
                if item_info:
                    ...
                    product_name = item_info.get('products_name') # Retain for other uses if needed
                    ...
                    if category_id_for_bin is None:
                        ...
                        # *** OLED if category not found (2 lines) ***
                        send_oled_text(["No Category", "Contact Staff"])
                        conn.close()
                        return redirect(url_for('bin', barcode_id_filter=barcode_id_to_search))
                    
                    # --- New logic: Check if disquantity already equals quantity ---
                    if current_disquantity >= current_quantity:
                        ...
                        # *** OLED for quantity exceeded (3 lines) ***
                        send_oled_text([
                            f"Item '{product_name[:14]}'", # Line 1: Product name (trimmed)
                            "Qty Exceeded.",               # Line 2: Quantity exceeded warning
                            "Contact staff."               # Line 3: Instruction
                        ])
                        print(f"[Warning] Cannot add disquantity for '{product_name}'. Already at max quantity.")
                        conn.close()
                        return redirect(url_for('bin', barcode_id_filter=barcode_id_to_search))
                    # --- End new logic ---
                    ...
                    session['stored_item_for_increment'] = {
                        ...
                        # Removed 'products_name' as it's not needed for OLED display here
                    }
                    current_item_awaiting_drop = { # Set global variable for sensor
                        'barcode_id': barcode_id_to_search,
                        'products_id': products_id_to_disquantity,
                        'category_id': category_id_for_bin,
                        'products_name': product_name # Retain product name for logging/other internal uses
                    }
                    
                    # Store current barcode in session for continuous OLED display
                    session['current_receipt_barcode'] = barcode_id_to_search
                    session['item_drop_pending'] = True # Mark that an item is awaiting disposal
                    ...
                    conn.commit()
                    # *** OLED Display: Product ID and category for disposal (2 lines) ***
                    send_oled_text([
                        f"Item: {products_id_to_disquantity[:14]}",  # Line 1: Product ID (trimmed)
                        f"Drop in Bin: {category_id_for_bin}" # Line 2: Bin instruction
                    ])
                else:
                    flash("No matching product found.", 'danger')
                    # *** OLED for no matching product (2 lines) ***
                    send_oled_text(["No Product", "Found"])
            except mysql.connector.Error as err:
                flash(f"Error during operation: {err}", 'danger')
                conn.rollback()
                # *** OLED for DB error (2 lines) ***
            ...
            # Retrieve barcode ID from form to preserve it after redirect
            ...
            if not conn_reset:
                ...
                # *** OLED for DB error during reset (2 lines) ***
                send_oled_text(["DB Disconnected", "Error!"])
            ...
            try:
                ...
                # Call the shared logic for disquantity update and session/global variable management
                # This call is intended for button press, will not trigger sensor-specific OLED messages
                ...
                # Send OFF commands to all Servos on ESP32 directly from button press
                ...
                # *** OLED Display for manual reset (2 lines) ***
                send_oled_text([
                    "System Reset.",      # Line 1: Status
                    "Ready for scan."     # Line 2: Instruction
                ])
                time.sleep(2) # Display for 2 seconds
                # *** OLED Display: Return to initial state (2 lines) ***
                send_oled_text(["Scan Receipt", "Barcode"]) # Return to initial state
                session.pop('current_receipt_barcode', None) # Clear session on full reset
                session.pop('stored_item_for_increment', None) # Clear item awaiting drop from button
                session['item_drop_pending'] = False
    ...
    # Handle GET requests (including 'reset_search' and initial page load)
    if request.method == 'GET':
        action = request.args.get('action')
        
        if action == 'reset_search':
            # Action when user clicks 'Reset' button in the search section
            print("[Reset Search Action] Clearing search filter, resetting servos, and OLED.")
            barcode_id_filter = '' # Clear search filter
            request_form_data = {} # Clear previous form data
            ...
            try:
                ...
                # *** OLED Display: Initial prompt for scanning receipt barcode (2 lines) ***
                ...
                # *** OLED for general error during reset (2 lines) ***
        ...
        # --- Existing GET request logic (search and initial page load) ---
        ...
        if display_barcode_id:
            # Query to get all order items with the same barcode_id
            base_query = """
            ...
            if orders_data and totals and totals['total_quantity'] is not None:
                # If an item is awaiting disposal, do not change OLED from 'DROP IN BIN'
                # This keeps the disposal instruction on the OLED
                if not session.get('item_drop_pending'):
                    # Update OLED only when no active item is awaiting disposal
                    ...
                    # CORRECTED: Each piece of information now on its own line
                    send_oled_text([
                        f"Barcode:{display_barcode_id[:12]}", # Line 1: Barcode ID
                        f"Rem:{remaining}",                     # Line 2: Remaining quantity
                        f"Total:{total_quantity}",              # Line 3: Total quantity
                        f"Refunded:{total_disquantity}"         # Line 4: Refunded quantity
                    ])
                ...
                # No order items found for this barcode filter (or empty data)
                # *** OLED for no order items found (2 lines) ***
            ...
            orders_data = []
            # *** OLED Display: Initial prompt for scanning receipt barcode when no filter is active (2 lines) ***
            send_oled_text(["Scan Receipt", "Barcode"])
            session.pop('current_receipt_barcode', None)
            session['item_drop_pending'] = False
            current_item_awaiting_drop = None # Clear global variable
    ...
    # This block will only run for normal GET requests (not 'reset_search' or 'search' POST)
    if 'conn' in locals() and conn.is_connected(): # Check if connection is still open from previous GET attempt
        conn.close() # Close connection if still open from previous GET attempt
        conn = get_db_connection() # Reopen if necessary for final render data
        if conn:
            cursor = conn.cursor(dictionary=True)
            # Fetch Servo data again for final render, in case it was updated by a reset
            cursor.execute("SELECT id, value FROM tbl_bin")
            servo_rows = cursor.fetchall()
            servo_data = {row['id']: row['value'] for row in servo_rows}
        ...
        if not servo_data: # If servo_data is still empty, try to fetch again if possible
    ...
    current_auto_order_id = "N/A"  # This variable does not seem to be used in the prompt's context
    return render_template("bin.html",
                           orders=orders_data,
                           barcode_id_filter=display_barcode_id, # Use display_barcode_id for form input field
