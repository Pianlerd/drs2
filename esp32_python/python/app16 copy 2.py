        ...
        if order_item_to_update:
            ...
            if proposed_disquantity <= current_quantity:
                ...
                if is_sensor_trigger:
                    ...
                    # Check if there are more items to refund for this receipt
                    cursor.execute("""
                    ...
                    if current_totals and current_totals['total_quantity'] is not None and \
                       current_totals['total_quantity'] > current_totals['total_disquantity']:
                        # More items still need to be refunded for this receipt
                        send_oled_text(["SELECT NEXT ITEM", "TO DROP"])
                        print(f"[OLED Update] More items for barcode {barcode_id_to_search} remaining. Prompting for next item.")
                        session['item_drop_pending'] = False # Current item dropped, ready to select next
                    else:
                        # All items for this receipt have been refunded
                        send_oled_text(["SCAN RECEIPT", "BARCODE"])
                        print(f"[OLED Update] All items for barcode {barcode_id_to_search} refunded. Prompting for new receipt.")
                        # Clear the global variable and session state as the receipt is fully processed
                        current_item_awaiting_drop = None
...
def bin():
    ...
    """
    global current_item_awaiting_drop # Declare global to modify it
    ...
    if request.method == 'POST':
        ...
        if action == 'add_disquantity':
            ...
            if barcode_id_filter:
                # Store current barcode in session
                session['current_receipt_barcode'] = barcode_id_filter
                session['item_drop_pending'] = False # Not yet selected an item to drop
                ...
                orders_data_for_display = cursor.fetchall()
                ...
                if orders_data_for_display and totals and totals['total_quantity'] is not None:
                    total_quantity = totals['total_quantity']
                    total_disquantity = totals['total_disquantity']
                    remaining = total_quantity - total_disquantity
                    # OLED Display: Barcode summary
                    send_oled_text([
                        f"BARCODE:{barcode_id_filter[:12]}", # Truncate barcode for OLED
                        f"REM:{remaining} TOTAL:{total_quantity}",
                        f"REFUNDED:{total_disquantity}"
                    ])
                    # After showing summary, prompt to select item to drop
                    time.sleep(2) # Display summary for 2 seconds
                    send_oled_text(["SELECT ITEM", "TO DROP"]) # Changed from SCAN ITEM
                else:
                    # OLED Display: No order found for barcode
                    send_oled_text(["NO ORDER FOUND", "SCAN NEW RECEIPT"])
                    session.pop('current_receipt_barcode', None) # Clear session if no order
                    session['item_drop_pending'] = False
            else:
                # If search button pressed but no barcode entered
                send_oled_text(["SCAN RECEIPT", "BARCODE"])
                session.pop('current_receipt_barcode', None)
                session['item_drop_pending'] = False
            conn.close() # Close connection after search if not already closed
