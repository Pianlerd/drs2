    ...
    if request.method == 'POST':
        ...
        if action == 'add_disquantity':
            ...
            try:
                ...
                if item_info:
                    # ===== START: MODIFICATION FOR QUANTITY CHECK =====
                    current_quantity = item_info.get('quantity')
                    current_disquantity = item_info.get('disquantity')
                    product_name = item_info.get('products_name', 'Unknown')

                    if current_disquantity >= current_quantity:
                        flash(f"สินค้ารายการ '{product_name}' ถูกคืนครบจำนวนแล้ว", 'info')
                        send_oled_text([
                            f"{product_name[:20]}", 
                            "ALREADY REFUNDED"
                        ])
                        ...
                        session['item_drop_pending'] = False
                        conn.close()
                        return redirect(url_for('bin')) # Redirect to reset the page
                    # ===== END: MODIFICATION FOR QUANTITY CHECK =====
