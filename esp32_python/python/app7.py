        ...
        try:
            ...
            if order_item_to_update:
                ...
                if proposed_disquantity <= current_quantity:
                    ...
                    # UPDATED: Set value in tbl_bin to 1 where category_id matches the product's category_id
                    # This assumes tbl_bin has a 'category_id' column and a 'value' column.
                    cursor.execute("UPDATE tbl_bin SET value = 1 WHERE category_id = %s",
                                   (category_id_for_bin,))
