...
def bin():
    ...
    if request.method == 'POST':
        ...
        if request.form.get('action') == 'add_disquantity':
            ...
            try:
                ...
                if stored_item:
                    ...
                    if order_item_to_update:
                        ...
                        if proposed_disquantity <= current_quantity:
                            ...
                            # Reset all LEDs for category_id 1-5 to 0
                            ...
                            flash(f"เพิ่มจำนวนทิ้งสินค้า '{product_name}' (รหัสสินค้า: {products_id_to_disquantity}) สำเร็จ และรีเซ็ตสถานะ LED แล้ว.", 'success')
                        else:
                            flash(f"ไม่สามารถเพิ่มจำนวนทิ้งได้เกินจำนวนสินค้าที่มีอยู่ ({current_quantity} ชิ้น) สำหรับสินค้า '{product_name}'", 'danger')
                            # Still reset LEDs even if disquantity not incremented due to limit
                            cursor_reset.execute("UPDATE tbl_bin SET value = 0 WHERE category_id IN (1, 2, 3, 4, 5)")
                            conn_reset.commit()
                            session.pop('stored_item_for_increment', None)
                            session['first_click_done'] = False
                        ...
                        # If no stored item found, just reset LEDs
                        cursor_reset.execute("UPDATE tbl_bin SET value = 0 WHERE category_id IN (1, 2, 3, 4, 5)")
                        conn_reset.commit()
                        session.pop('stored_item_for_increment', None)
                        session['first_click_done'] = False
                    ...
                    flash("ไม่มีสินค้าที่ถูกเลือกไว้สำหรับการเพิ่มจำนวนทิ้ง. กำลังรีเซ็ตสถานะ LED เท่านั้น.", 'info')
                    # If no stored item, just reset LEDs
                    cursor_reset.execute("UPDATE tbl_bin SET value = 0 WHERE category_id IN (1, 2, 3, 4, 5)")
                    conn_reset.commit()
                    session.pop('stored_item_for_increment', None)
                    session['first_click_done'] = False
                ...
                flash(f"เกิดข้อผิดพลาดในการรีเซ็ต LED และเพิ่มจำนวนทิ้ง: {err}", 'danger')
                conn_reset.rollback()
            finally:
                if conn_reset:
                    conn_reset.close()
    ...
    # --- Logic for handling 'add_disquantity' (First Click to activate LED) ---
    if request.method == "POST" and request.form.get('action') == 'add_disquantity':
        ...
        try:
            ...
            if item_info:
                ...
                # Turn on the specific LED and turn off others (category_id 1-5)
                ...
                conn.commit()
                flash(f"ไฟ LED สำหรับสินค้า '{product_name}' (category_id: {category_id_for_bin}) ติดแล้ว. กรุณากดปุ่ม 'รีเซ็ตสถานะ LED (category_id 1-5)' เพื่อยืนยันการเพิ่มจำนวนทิ้ง.", 'info')
            else:
