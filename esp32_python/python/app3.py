...
def _handle_disquantity_logic(conn, cursor, is_sensor_trigger=False, flash_func=None):
    ...
    if item_to_process:
        ...
        product_name = item_to_process.get('products_id', 'UNKNOWN ITEM') # Get product name from stored item

        # Fetch the order item to update its disquantity
        cursor.execute("""
            SELECT id, quantity, disquantity, products_id
            FROM tbl_order
        ...
        if order_item_to_update:
            ...
            product_name = order_item_to_update['products_id'] # Get from fetched data for accuracy
...
def bin():
    ...
    if request.method == 'POST':
        ...
        if action == 'add_disquantity':
            ...
            try:
                ...
                cursor.execute("""
                    SELECT p.category_id, o.products_id, o.quantity, o.disquantity
                    FROM tbl_order o
                ...
                if item_info:
                    ...
                    product_name = item_info.get('products_id')
                    ...
                    if current_disquantity >= current_quantity:
                        flash(f"Cannot add discarded quantity for '{product_name}'. All items for this product have already been discarded.", 'danger')
                        send_oled_text([
                            f"ITEM '{product_name[:14]}'",
                            "EXCEEDS QUANTITY.",
                            "CONTACT STAFF."
                        ]) # Truncate product name for OLED
                    ...
                    current_item_awaiting_drop = { # Set global variable for sensor
                        ...
                        'products_id': products_id_to_disquantity,
                        'category_id': category_id_for_bin,
                        'products_id': product_name # Store product name here for sensor to use
                    }
    ...
    if request.method == 'GET':
        ...
        if display_barcode_id:
            ...
            base_query = """
                SELECT o.*, p.price, p.products_id, p.category_id
                FROM tbl_order o
                JOIN tbl_products p ON o.products_id = p.products_id
    ...
    return render_template("bin.html",
                           ...
                           servo_data=servo_data, state=button_state)
...
@app.route("/apii/sscan_barcode", methods=["POST"])
def api_sscan_barcode():
    """
    รับ barcode ทาง POST (form หรือ JSON).
    หา product แรกที่ยังเหลือ (quantity > disquantity) ใน tbl_order สำหรับ barcode นั้น
    -> ตั้ง current_item_awaiting_drop, อัปเดต tbl_bin เพื่อเปิด bin ตาม category_id
    -> ส่ง OLED ข้อความ และคืน JSON สถานะกลับ
    """
    global current_item_awaiting_drop

    # อ่าน barcode จาก form หรือ JSON
    barcode = request.form.get("barcode") or (request.json and request.json.get("barcode"))
    if not barcode:
    ...
    try:
        ...
        # หา product แรกในออร์เดอร์ที่ยังเหลือ (quantity > disquantity)
        cursor.execute("""
            SELECT o.id AS order_row_id, o.products_id, o.quantity, o.disquantity, p.category_id
            FROM tbl_order o
            JOIN tbl_products p ON o.products_id = p.products_id
            WHERE o.barcode_id = %s AND (o.quantity - o.disquantity) > 0
            ORDER BY o.id ASC
            LIMIT 1
        """, (barcode,))
        row = cursor.fetchone()
        ...
        if not row:
            # ถ้าไม่พบสินค้าเหลือ ให้ตอบว่าไม่มี order/สินค้า
            # เคลียร์สถานะที่เกี่ยวข้อง (ถ้าต้องการ)
            current_item_awaiting_drop = None
            conn.close()
            send_oled_text(["NO ORDER FOUND", "SCAN NEW RECEIPT"])
            return jsonify({"status": "not_found", "message": "no remaining product for barcode"}), 404

        # ถ้าพบสินค้า: ตั้ง current_item_awaiting_drop และอัปเดต tbl_bin เพื่อเปิด bin ตาม category
        products_id = row['products_id']
        order_row_id = row['order_row_id']
        category_id = row['category_id']

        # อัปเดตฐานข้อมูล tbl_bin: ปิดทุก category อื่นที่เป็น 1-5 แล้วเปิด category ของสินค้า
        cursor.execute("UPDATE tbl_bin SET value = 0 WHERE category_id != %s AND category_id IN (1,2,3,4,5)", (category_id,))
        cursor.execute("UPDATE tbl_bin SET value = 1 WHERE category_id = %s", (category_id,))
        conn.commit()

        # ตั้ง current_item_awaiting_drop ให้มีรายละเอียดที่ _handle_disquantity_logic_ ต้องการ
        current_item_awaiting_drop = {
            'barcode_id': barcode,
            'products_id': products_id,
            'category_id': category_id,
            'products_name': products_id  # ถ้าต้องการชื่อจริงให้ query แล้วใส่
        }

        # เก็บ barcode ใน session เพื่อให้ UI แสดงผล (ถ้ามี browser เปิด)
        session['current_receipt_barcode'] = barcode
        session['item_drop_pending'] = True

        # OLED แจ้งผู้ใช้ให้ทิ้งสินค้า
        send_oled_text([f"ITEM:{str(products_id)[:14]}", f"DROP IN BIN:{category_id}"])
        ...
        return jsonify({
            ...
            "message": "item selected, bin opened"
        }), 200
        ...
        try:
            ...
            pass
        conn.close()
        print(f"[API ERROR] scan_barcode: {e}")
...
def scan_barcode():
    ...
    print("รับ Barcode:", barcode)
    # ประมวลผลต่อที่นี่ (database, servo, ฯลฯ)
