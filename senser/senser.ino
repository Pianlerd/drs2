...
const int delayTime = 300;    // Delay in milliseconds (used as interval for non-blocking)
...
// Flask Server IP
const char* flask_ip = "192.168.1.141";  // ต้องตรงกับ IP ปัจจุบันของคอมพิวเตอร์
...
void handleNewServoAndButton() {
  ...
  if (digitalRead(newButtonPin) == LOW && newServoState == STATE_IDLE && totalRounds > 0) {
